...
void tutorial_threadpool_1(UThreadPoolPtr tp) {
	...
	auto r1 = tp->commit(
	    [i, j] { return add(i, j); }); // 可以通过lambda表达式传递函数
	std::future<float> r2 =
	    tp->commit(std::bind(minusBy5, 8.5f)); // 可以传入任意个数的入参
	auto r3 = tp->commit(
	    std::bind(&MyFunction::concat, mf, str)); // 返回值可以是任意类型
	std::future<int> r4 = tp->commit([i, j] {
		return MyFunction::multiply(i, j);
	}); // 返回值实际上是std::future<T>类型

	std::cout << r1.get() << std::endl; // 返回值可以是int类型
	std::cout
	    << r2.get()
	    << std::
	           endl; // 等待r2对应函数执行完毕后，再继续执行。不调用get()为不等待
...
void tutorial_threadpool_2(UThreadPoolPtr tp) {
	...
	taskGroup.addTask([i, j] {
		...
		ECHO("sleep for 1 second, [%d] + [%d] = [%d], run success.", i,
		            j, result);
	});
	...
	taskGroup.addTask([i, j, k] {
		...
		ECHO(
		    "sleep for 2 second, [%d] - [%d] + [%d] = [%d], run success.", i, j,
		    k, result);
	...
	/* taskGroup.setOnFinished([] (const CStatus& sts) {
	    if(sts.isOK()) {
	        ...
	        CGRAPH_ECHO("task group run failed, error info is [%s].",
	sts.getInfo().c_str());
	    }
	}); */
...
void tutorial_threadpool_3(UThreadPoolPtr tp) {
	...
	ECHO("thread pool task commit version : ");
	for (int i = 0; i < size; i++) {
...
int main() {
	std::unique_ptr<UThreadPool> pool(
	    new UThreadPool()); // 构造一个线程池类的智能指针
